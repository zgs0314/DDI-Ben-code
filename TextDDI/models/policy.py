...
class Policy(nn.Module):
    ...
    def return_probs_list(self,action_input_ids,action_attention_mask,state_input_ids,state_attention_mask,drug1_sent_num,drug2_sent_num):
        ...
        action_encodings = self.llm(action_input_ids,action_attention_mask, output_hidden_states = True).hidden_states[-1][:,0,:]
        state_encodings = self.llm(state_input_ids,state_attention_mask, output_hidden_states = True).hidden_states[-1][:,0,:]
    ...
    def evaluate_actions(self,
        ...
        state_encodings = self.llm(obs_input_ids_batch,obs_attention_mask_batch, output_hidden_states = True).hidden_states[-1][:,0,:]
    ...
    def act(self, 
        ...
        action_encodings = self.llm(filter_action_input_ids,filter_action_attention_mask, output_hidden_states = True).hidden_states[-1][:,0,:]

        action_encodings = self.action_tran(action_encodings)

        # state_encodings = self.llm(filter_state_input_ids,filter_state_attention_mask).last_hidden_state[:,0,:]
        state_encodings = self.llm(filter_state_input_ids,filter_state_attention_mask, output_hidden_states = True).hidden_states[-1][:,0,:]
