...
class twosides_dataset_rl(Dataset):
    def __init__(self,args,state,dtp = None, adv = None):
        with open(args.info_dict_path, 'r') as file:
            self.info_dict = json.load(file)
        ### new part
        self.data_path = dtp
        # used_set = [j for j in self.info_dict]
        with open('./data/twosides_{}/cid2id.json'.format(self.data_path), 'r') as file:
            new_cid2id = json.load(file)
        true_cid2id = {str(int(j[3:])):new_cid2id[j] for j in new_cid2id}
        # filter_set = [true_cid2id[j] for j in used_set]
        # self.cid2id = {j:str(true_cid2id[j]) for j in used_set}
        # self.id2cid = {str(true_cid2id[j]):j for j in used_set}
        # self.id2cidd = {new_cid2id[j]:j for j in new_cid2id}

        self.cid2id = {j:str(true_cid2id[j]) for j in true_cid2id}
        self.id2cid = {str(true_cid2id[j]):j for j in true_cid2id}

        self.data = [] ### here to see which is lacked [i for i in range(645) if i not in filter_set]
        with open('./data/twosides_{}/{}.txt'.format(self.data_path, state), 'r') as file:
            flag = 0
            temp = []
            for j in file:
                h,t,r,p = j[:-1].split(' ')
