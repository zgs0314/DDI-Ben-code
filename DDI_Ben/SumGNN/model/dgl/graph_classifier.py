...
class GraphClassifier(nn.Module):
    def __init__(self, params, relation2id):  # in_dim, h_dim, rel_emb_dim, out_dim, num_rels, num_bases):
        ...
        if self.params.adversarial:
            self.cdan_dim = self.fc_layer.in_features
        #print(self.fc_layer)
    ...
    def forward(self, data):
        g = data
        # print("Before GNN Layer:", g.ndata)
        ...
        final_layer = F.dropout(g_rep, p=0.3)
        output = self.fc_layer(final_layer)
        # print(head_ids.detach().cpu().numpy(), tail_ids.detach().cpu().numpy())
        if self.params.adversarial:
            return output, final_layer
        else:
            return output
