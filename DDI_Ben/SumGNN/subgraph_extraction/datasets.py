...
def generate_subgraph_datasets(params, splits=['train', 'valid_S0', 'test_S0', 'valid_S1', 'test_S1', 'valid_S2', 'test_S2'], saved_relation2id=None, max_label_value=None):
    ...
    triple_file = 'data/{}/KG.txt'.format(params.dataset)
    ...
    if testing:
        ...
        save_to_file(directory, f'neg_{params.test_S0_file}_{params.constrained_neg_prob}.txt', graphs[params.test_S0_file]['neg'], id2entity, id2relation)
        save_to_file(directory, f'neg_{params.test_S1_file}_{params.constrained_neg_prob}.txt', graphs[params.test_S1_file]['neg'], id2entity, id2relation)
        save_to_file(directory, f'neg_{params.test_S2_file}_{params.constrained_neg_prob}.txt', graphs[params.test_S2_file]['neg'], id2entity, id2relation)
...
class SubgraphDataset(Dataset):
    ...
    def __init__(self, db_path, db_name_pos, db_name_neg, raw_data_paths, included_relations=None, add_traspose_rels=False, num_neg_samples_per_link=1, use_kge_embeddings=False, dataset='', kge_model='', file_name='', \
        ...
        triple_file = 'data/{}/KG.txt'.format(dataset)
        self.entity_type = np.loadtxt('data/{}/entity.txt'.format(dataset))

        if not ssp_graph:
            if not dataset.startswith('drugbank'):
