...
from models.roberta_env import roberta_env
from models.policy import Policy
from drugbank.train import train
import time
...
def main():
    ...
    config_file = './configs/ppo_drugbank.yaml'# parser.parse_args().config_file
    ...
    if args.eval:
        log_path = './log/{}/{}-eval.log'.format(start_date + '_drppo',time.strftime("%Y-%m-%d %H:%M:%S",time.localtime()))
    else:
        log_path = './log/{}/{}.log'.format(start_date + '_drppo', time.strftime("%Y-%m-%d %H:%M:%S",time.localtime()))
    ...
    env_model.load_state_dict({k.replace('module.',''):v for k,v in initialize_checkpoint['model'].items()},strict=False)
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        global_step,eval_time = train(args,train_dataloader,actor_critic,env_model,optimizer,writer,logger,global_step,eval_time,start_date)
        torch.cuda.empty_cache()
        gc.collect()
