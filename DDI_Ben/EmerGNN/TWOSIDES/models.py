...
class EmerGNN(nn.Module):
    def __init__(self, eval_ent, eval_rel, args):
        ...
        if args.feat == 'M':
            ...
            self.Went = nn.Linear(len(mfeat[0]), args.n_dim)
        ...
        if args.adversarial:
            if args.feat == 'E':
                self.random_layer = network.RandomLayer([4*args.n_dim, 2], 500)
            else:
                self.random_layer = network.RandomLayer([2*args.n_dim, 2], 500)
            self.ad_net = network.AdversarialNetwork(500, 500)
