...
class Decagon(nn.Module):
    ...
    def forward(self, data):
        ...
        # head_emb = self.batchnorm(head_emb)
        # tail_emb = self.batchnorm(tail_emb)
        ...
        final_layer = torch.cat([head_emb, tail_emb], dim=1)

        # head_trans = self.batchnorm(head_trans)
        # tail_trans = self.batchnorm(tail_trans)
        ...
        # predict = torch.sigmoid(predict).T
        predict = predict.T
        # predict = torch.softmax(predict.T, dim=1)
        # predict = torch.sigmoid(predict)
        if self.args.adversarial:
            return predict, final_layer
        else:
            return predict
