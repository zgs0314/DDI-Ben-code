...
class CompGCNBase(BaseModel):
	def __init__(self, edge_index, edge_type, num_rel, params=None, feat = None):
		...
		# 	self.init_rel  = get_param((self.p.num_bases,   self.p.comp_dim))
		# else:
		# if self.p.Comp_sfunc == 'TransE': 	self.init_rel = get_param((num_rel,   self.p.comp_dim))
		# else: 					self.init_rel = get_param((num_rel*2, self.p.comp_dim))

		# if self.p.num_bases > 0:
		# 	self.conv1 = CompGCNConvBasis(self.p.comp_dim, self.p.comp_dim, num_rel, self.p.num_bases, act=self.act, params=self.p)
	...
	def forward_base_ddi(self, sub, obj, drop1, drop2):
		# r	= self.init_rel if self.p.Comp_sfunc != 'TransE' else torch.cat([self.init_rel, -self.init_rel], dim=0)
		r	= self.init_rel 
		# if self.p.Comp_sfunc != 'TransE' else torch.cat([self.init_rel, -self.init_rel], dim=0)
		x, r	= self.conv1(self.init_embed, self.edge_index, self.edge_type, rel_embed=r)
		x	= drop1(x)
		x, r	= self.conv2(x, self.edge_index, self.edge_type, rel_embed=r) 	if self.p.gcn_layer == 2 else (x, r)
		x	= drop2(x) 							if self.p.gcn_layer == 2 else x
...
class CompGCN_TransE_DDI(CompGCNBase):
	...
	# def forward(self, sub, obj): ### specially for obj
...
class CompGCN_DistMult_DDI(CompGCNBase):
	def __init__(self, edge_index, edge_type, params=None):
		super(self.__class__, self).__init__(edge_index, edge_type, params.num_rel, params)
		self.drop = torch.nn.Dropout(self.p.comp_drop)
	...
	def forward(self, sub, obj):
		...
		sub_emb, obj_emb, all_ent	= self.forward_base_ddi(sub, obj, self.drop, self.drop)
		rel_emb	= obj_emb / (sub_emb + 0.000001) 

		x = torch.mm(rel_emb, all_ent.transpose(1, 0))/10000
		# x += self.bias.expand_as(x)
