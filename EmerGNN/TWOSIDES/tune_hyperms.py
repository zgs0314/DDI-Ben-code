...
if __name__ == '__main__':
        ...
        try:
            for e in range(args.n_epoch):
                ...
                if (e+1) % args.epoch_per_test == 0:
                    ...
                    out_str = time_now + ':epoch:%d\tfeat:%s lr:%.6f lamb:%.8f n_batch:%d n_dim:%d layer:%d\t%.4f %.4f %.4f\t %.4f %.4f %.4f' % (e+1, args.feat, args.lr, args.lamb, args.n_batch, args.n_dim, args.length, v_roc, v_pr, v_ap, t_roc, t_pr, t_ap)
                    if v_pr > best_acc:
                        best_acc = v_pr
                        best_str = out_str
                        early_stop = 0
    ...
    space = {
        "lr": hp.choice("lr", [1e-2, 3e-3, 1e-3, 3e-4, 1e-4]),
        "lamb": hp.choice("lamb", [1e-8, 1e-6, 1e-4]),
