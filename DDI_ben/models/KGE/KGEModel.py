...
class KGEModel(nn.Module):
    def __init__(self, model_name, nentity, nrelation, args):
        ...
        self.cdan_dim = self.hidden_dim*2
    ...
    def forward(self, sample):
        ...
        if 'valid' in mode or 'test' in mode:
            if self.model_name == 'ComplEx':
                ...
                # if self.args.dataset == 'twosides':
                # #     # return pos_score/pos_score.max(0)[0]
                # #     return torch.sigmoid(pos_score - pos_score.mean(0))
                # # else:
                    return pos_score
        else:
            if self.args.adversarial:
                if self.model_name == 'ComplEx':
                    pos_relation_emb = self.dropout(self.relation_embedding[torch.tensor([i for i in range(int(self.nrelation/2))]).repeat(pos_data.shape[0],1).to(self.args.device)])
                else:
                    pos_relation_emb = self.dropout(self.relation_embedding[torch.tensor([i for i in range(int(self.nrelation))]).repeat(pos_data.shape[0],1).to(self.args.device)])
                pos_score_label = self.model_func[self.model_name](head, pos_relation_emb, tail, inv_relation_mask=inv_relation_mask, mode=mode)
                final_layer = torch.cat([head, tail], dim=1)
                if self.model_name == 'ComplEx':
                    return [pos_score, neg_score], final_layer, pos_score_label.transpose(0,1)
                else:
                    return [pos_score, neg_score], final_layer, pos_score_label
            else:
    ...
    def loss(self, pred, true_label):
        ...
        if self.model_name == 'MSTE':
            ...
            # loss = self.softplus((self.gamma + negative_score - positive_score)).mean()
            # loss = torch.sigmoid(self.gamma + negative_score - positive_score).mean()
            return loss
        ...
        #     pos_loss = self.BCELoss(positive_score, torch.ones(positive_score.size()).to(args.device)).squeeze(-1)                          # [B]
        #     neg_loss = self.BCELoss(negative_score, torch.zeros(negative_score.size()).to(args.device))                                     # [B, N_neg]
