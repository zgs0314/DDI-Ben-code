...
def process_files_ddi(files, triple_file, saved_relation2id=None, keeptrainone = False):
    ...
    # 假设 entity2id 已经构建完成
    missing_entities = []
    for i in range(34124):  # 从 0 到 34123
...
def process_files_decagon(files, triple_file, saved_relation2id=None, keeptrainone = False):
    ...
    for i in range(rel):
        idx = np.argwhere(triplets['train'][:, 2] == i)
        adj_list.append(csc_matrix((np.ones(len(idx), dtype=np.uint8), (triplets['train'][:, 0][idx].squeeze(1), triplets['train'][:, 1][idx].squeeze(1))), shape=(max(entity2id.values())+1, max(entity2id.values())+1)))
    for i in range(rel, len(relation2id)):
        idx = np.argwhere(kg_triple[:, 2] == i-rel)
        #print(len(idx), i)
        adj_list.append(csc_matrix((np.ones(len(idx), dtype=np.uint8), (kg_triple[:, 0][idx].squeeze(1), kg_triple[:, 1][idx].squeeze(1))), shape=(max(entity2id.values())+1, max(entity2id.values())+1)))
    #print(adj_list)
    #assert 0
