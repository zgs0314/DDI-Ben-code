...
def initialize_experiment(params, file_name):
    ...
    if file_name == 'test_auc.py':
        ...
        file_handler = logging.FileHandler(os.path.join(params.test_exp_dir, f"log_{params.valid_file[-2:]}_test.txt"))
    else:
...
def initialize_model(params, model, load_model=False):
    ...
    # assert 0
    best_graph_classifier_path = f'best_graph_classifier_{params.setting}' + ('_adv' if params.adversarial else '') + '.pth'
    if load_model and os.path.exists(os.path.join(params.exp_dir, best_graph_classifier_path)):
        logging.info('Loading existing model from %s' % os.path.join(params.exp_dir, best_graph_classifier_path))
        graph_classifier = torch.load(os.path.join(params.exp_dir, best_graph_classifier_path)).to(device=params.device)
    else:
