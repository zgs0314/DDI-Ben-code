...
def main(params):
    ...
    logging.info(f"Generate subgraph: time cost [{time.time() - start_time}]")
    ...
    if params.adversarial:
        valid_adv = SubgraphDataset(params.db_path, 'valid_S1_pos', 'valid_S1_neg', params.file_paths,
                                add_traspose_rels=params.add_traspose_rels,
                                num_neg_samples_per_link=params.num_neg_samples_per_link,
                                use_kge_embeddings=params.use_kge_embeddings, dataset=params.dataset,
                                kge_model=params.kge_model)
        test_adv = SubgraphDataset(params.db_path, 'test_S1_pos', 'test_S1_neg', params.file_paths,
                                add_traspose_rels=params.add_traspose_rels,
                                num_neg_samples_per_link=params.num_neg_samples_per_link,
                                use_kge_embeddings=params.use_kge_embeddings, dataset=params.dataset,
                                kge_model=params.kge_model)
        train_adv = ConcatDataset([valid_adv, test_adv])
    params.num_rels = train.num_rels
    params.aug_num_rels = train.aug_num_rels
    params.inp_dim = train.n_feat_dim
    params.train_rels = 209 if not params.dataset.startswith('drugbank') else params.num_rels
    params.num_nodes = 91000 if not params.dataset.startswith('drugbank') else 35000
    ...
    valid_evaluator = Evaluator(params, graph_classifier, valid) if params.dataset.startswith('drugbank')  else Evaluator_ddi2(params, graph_classifier, valid)
    test_evaluator = Evaluator(params, graph_classifier, test) if params.dataset.startswith('drugbank')  else Evaluator_ddi2(params, graph_classifier, test)
    train_evaluator = Evaluator(params, graph_classifier, train) if params.dataset.startswith('drugbank') else Evaluator_ddi2(params, graph_classifier, train)
    
    if params.adversarial:
        train = [train, train_adv]
    ...
    with open(params.save_result, "a") as f:
        f.write('\n')
    print(f"Whole time cost: {time.time() - start_time}")

if __name__ == '__main__':

    logging.basicConfig(level=logging.INFO)

    parser = argparse.ArgumentParser(description='SumGNN model')
    ...
    params.save_result = os.path.join(params.exp_dir, f'{params.dataset}_{params.setting}' + ('_adv' if params.adversarial else '') + '.txt')
    ...
    params.move_batch_to_device = move_batch_to_device_dgl if params.dataset.startswith('drugbank') else move_batch_to_device_dgl_ddi2
